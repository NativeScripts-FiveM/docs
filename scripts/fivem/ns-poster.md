...
| Diagnose a rejected placement | `posterdiag [playerId]` in the server console, or `/posterdiag` as an admin |
...
### "I confirm the placement and it throws me back into the designer"

That is the client's way of saying **the server rejected the placement** — the
design is kept and the editor reopens so nothing is lost. The reason is sent to
the player as a notification; if your notification system swallows it, ask the
server instead:
...
It prints the detected framework, the economy/item/job gates and the player's
balance, job and cooldown — i.e. every value the placement check looks at. For
the full picture set `Config.Debug = true`, restart, and try again: every
rejection then logs a `PLACE reject: …` line naming the exact check that failed.
...
| `too far` / `dz too big` | The prop was placed further than `Config.PlacementMaxDistance` from the player. |
